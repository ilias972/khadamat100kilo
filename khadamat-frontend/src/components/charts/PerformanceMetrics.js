...
exports.PerformanceIndicators = PerformanceIndicators;
const react_1 = __importDefault(require("react"));
const recharts_1 = require("recharts");
const lucide_react_1 = require("lucide-react");
const ChartContainer_1 = require("./ChartContainer");
function PerformanceMetrics({ data, loading = false, error = null, className, showDetails = true, compact = false }) {
    const colors = {
        success: '#22c55e',
        warning: '#f59e0b',
        error: '#ef4444',
    ...
    const performanceScore = Math.round((data.completionRate * 0.3 +
    ...
    if (compact) {
        ...
        <div className="space-y-3">
          {metrics.slice(0, 3).map((metric) => {
                ...
                return (<div key={metric.key} className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <IconComponent className="w-4 h-4" style={{ color: isGood ? metric.color : colors.error }}/>
    ...
    return (<ChartContainer_1.ChartContainer title="Métriques de performance" subtitle="Indicateurs clés de réussite" loading={loading} error={error} className={className} height={400}>
      
      <div className="flex items-center justify-center mb-6">
        <div className="relative">
          <recharts_1.ResponsiveContainer width={120} height={120}>
            <recharts_1.RadialBarChart cx={60} cy={60} innerRadius={40} outerRadius={60} barSize={8} data={[{ value: performanceScore, fill: performanceScore >= 80 ? colors.success : colors.warning }]} startAngle={90} endAngle={-270}>
              <recharts_1.RadialBar dataKey="value" background={{ fill: colors.neutral }} cornerRadius={10} fill={performanceScore >= 80 ? colors.success : colors.warning}/>
      ...
      <div className="grid grid-cols-2 gap-4">
        {metrics.map((metric) => {
            ...
            const progress = Math.min((metric.value / metric.target) * 100, 100);
            const isGood = metric.inverse ? metric.value <= metric.target : metric.value >= metric.target;
            return (<div key={metric.key} className={`p-4 rounded-lg border-2 transition-all duration-200 ${isGood
                    ? 'border-green-200 bg-green-50/50'
                    : 'border-red-200 bg-red-50/50'}`}>
              <div className="flex items-center justify-between mb-2">
                <IconComponent className="w-5 h-5" style={{ color: isGood ? metric.color : colors.error }}/>
                {isGood ? (<lucide_react_1.CheckCircle className="w-4 h-4 text-green-500"/>) : (<lucide_react_1.XCircle className="w-4 h-4 text-red-500"/>)}
              </div>
      ...
      {showDetails && (<div className="mt-6 pt-4 border-t border-border-light">
...
function RadialProgress({ value, target, label, color = '#F97B22', size = 100, showPercentage = true }) {
    ...
    return (<div className="text-center">
      <recharts_1.ResponsiveContainer width={size} height={size}>
        <recharts_1.PieChart>
          <recharts_1.Pie data={data} cx={size / 2} cy={size / 2} startAngle={90} endAngle={-270} innerRadius={size * 0.4} outerRadius={size * 0.5} dataKey="value" stroke="none"/>
        </recharts_1.PieChart>
      </recharts_1.ResponsiveContainer>
...
function PerformanceIndicators({ data, className }) {
    const indicators = [
        {
            label: 'Complétion',
            value: data.completionRate,
    ...
    return (<div className={`grid grid-cols-4 gap-4 ${className}`}>
      {indicators.map((indicator) => (<div key={indicator.label} className="bg-gradient-to-br from-[rgba(250,247,242,0.8)] to-[rgba(255,255,255,0.5)] backdrop-blur-sm rounded-[16px] p-4 text-center">
