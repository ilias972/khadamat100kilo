...
const Textarea = ({ label, error, helperText, className, ...props }) => {
    ...
    return (<div className="space-y-1">
      ...
      <textarea className={(0, utils_1.cn)('block w-full rounded-[24px] border bg-[#EDEEEF] px-4 py-3 text-text-primary placeholder-text-muted focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-300 transition-all duration-200 resize-none', error && 'border-error-500 focus:border-error-500 focus:ring-error-500', className)} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} {...props}/>
