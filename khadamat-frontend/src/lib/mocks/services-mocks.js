...
exports.mockCompanyValues = exports.mockTeamMembers = exports.mockCompanyStats = exports.getPopularFAQItems = exports.searchSupportArticles = exports.getRelatedSupportArticles = exports.getPopularSupportArticles = exports.getSupportArticlesByCategory = exports.getSupportArticleBySlug = exports.mockFAQItems = exports.mockSupportArticles = exports.mockSupportCategories = exports.getProRevenueByMonth = exports.getProMonthlyTransactions = exports.getProActiveServices = exports.getProPendingBookings = exports.getProRecentBookings = exports.getProBookingsByStatus = exports.mockProEarningsStats = exports.mockProTransactions = exports.mockProServices = exports.mockProBookings = exports.mockProProfile = exports.getClientUpcomingBookings = exports.getClientRecentBookings = exports.getClientUnreadMessages = exports.getClientBookingsByStatus = exports.mockClientStats = exports.mockClientFavorites = exports.mockClientMessages = exports.mockClientBookings = exports.mockClient = exports.getRelatedBlogArticles = exports.getPopularBlogArticles = exports.getBlogArticlesByCategory = exports.getBlogArticleBySlug = exports.mockBlogArticles = exports.mockBlogCategories = exports.mockProfessionalDetails = exports.mockProfessionals = exports.mockCategories = exports.mockCities = void 0;
exports.mockCities = [
...
exports.mockProfessionalDetails = {
    '1': {
        ...exports.mockProfessionals[0],
...
const getBlogArticleBySlug = (slug) => {
    return exports.mockBlogArticles.find(article => article.slug === slug && article.isPublished);
};
exports.getBlogArticleBySlug = getBlogArticleBySlug;
const getBlogArticlesByCategory = (categorySlug) => {
    const category = exports.mockBlogCategories.find(cat => cat.slug === categorySlug);
    if (!category)
        return [];
    return exports.mockBlogArticles.filter(article => article.category === category.name && article.isPublished);
};
exports.getBlogArticlesByCategory = getBlogArticlesByCategory;
const getPopularBlogArticles = (limit = 5) => {
    return exports.mockBlogArticles
        .filter(article => article.isPublished)
        .sort((a, b) => b.views - a.views)
        .slice(0, limit);
};
exports.getPopularBlogArticles = getPopularBlogArticles;
const getRelatedBlogArticles = (currentArticle, limit = 3) => {
    return exports.mockBlogArticles
...
exports.getRelatedBlogArticles = getRelatedBlogArticles;
exports.mockClient = {
    id: 'client-1',
...
const getClientBookingsByStatus = (status) => {
    if (!status)
        return exports.mockClientBookings;
    return exports.mockClientBookings.filter(booking => booking.status === status);
};
exports.getClientBookingsByStatus = getClientBookingsByStatus;
const getClientUnreadMessages = () => {
    return exports.mockClientMessages.filter(message => !message.isRead && message.isFromClient === false);
};
exports.getClientUnreadMessages = getClientUnreadMessages;
const getClientRecentBookings = (limit = 5) => {
    return exports.mockClientBookings
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
        .slice(0, limit);
};
exports.getClientRecentBookings = getClientRecentBookings;
const getClientUpcomingBookings = () => {
    const now = new Date();
    return exports.mockClientBookings
        .filter(booking => {
...
exports.getClientUpcomingBookings = getClientUpcomingBookings;
exports.mockProProfile = {
    id: 'pro-1',
    fullName: 'Ahmed Bennani',
...
const getProBookingsByStatus = (status) => {
    if (!status)
        return exports.mockProBookings;
    return exports.mockProBookings.filter(booking => booking.status === status);
};
exports.getProBookingsByStatus = getProBookingsByStatus;
const getProRecentBookings = (limit = 5) => {
    return exports.mockProBookings
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
        .slice(0, limit);
};
exports.getProRecentBookings = getProRecentBookings;
const getProPendingBookings = () => {
    return exports.mockProBookings.filter(booking => booking.status === 'pending');
};
exports.getProPendingBookings = getProPendingBookings;
const getProActiveServices = () => {
    return exports.mockProServices.filter(service => service.isActive);
};
exports.getProActiveServices = getProActiveServices;
const getProMonthlyTransactions = (month) => {
    if (!month) {
        const now = new Date();
        const currentMonth = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
        return exports.mockProTransactions.filter(txn => txn.createdAt.startsWith(currentMonth));
    }
    return exports.mockProTransactions.filter(txn => txn.createdAt.startsWith(month));
};
exports.getProMonthlyTransactions = getProMonthlyTransactions;
const getProRevenueByMonth = () => {
    return [
...
exports.getProRevenueByMonth = getProRevenueByMonth;
exports.mockSupportCategories = [
    {
        id: '1',
        name: 'Réservations',
        slug: 'reservations',
...
const getSupportArticleBySlug = (slug) => {
    return exports.mockSupportArticles.find(article => article.slug === slug && article.isPublished);
};
exports.getSupportArticleBySlug = getSupportArticleBySlug;
const getSupportArticlesByCategory = (categorySlug) => {
    const category = exports.mockSupportCategories.find(cat => cat.slug === categorySlug);
    if (!category)
        return [];
    return exports.mockSupportArticles.filter(article => article.categoryId === category.id && article.isPublished);
};
exports.getSupportArticlesByCategory = getSupportArticlesByCategory;
const getPopularSupportArticles = (limit = 5) => {
    return exports.mockSupportArticles
        .filter(article => article.isPublished)
        .sort((a, b) => b.views - a.views)
        .slice(0, limit);
};
exports.getPopularSupportArticles = getPopularSupportArticles;
const getRelatedSupportArticles = (currentArticle, limit = 3) => {
    return exports.mockSupportArticles
        .filter(article => article.id !== currentArticle.id &&
        article.isPublished &&
...
exports.getRelatedSupportArticles = getRelatedSupportArticles;
const searchSupportArticles = (query) => {
    if (!query.trim())
        return exports.mockSupportArticles.filter(article => article.isPublished);
    const searchTerm = query.toLowerCase();
    return exports.mockSupportArticles
        .filter(article => article.isPublished && (article.title.toLowerCase().includes(searchTerm) ||
        article.content.toLowerCase().includes(searchTerm) ||
        article.tags.some(tag => tag.toLowerCase().includes(searchTerm)) ||
        article.categoryName.toLowerCase().includes(searchTerm)));
};
exports.searchSupportArticles = searchSupportArticles;
const getPopularFAQItems = (limit = 5) => {
    return exports.mockFAQItems
        .filter(faq => faq.isPopular)
        .sort((a, b) => b.views - a.views)
        .slice(0, limit);
};
exports.getPopularFAQItems = getPopularFAQItems;
exports.mockCompanyStats = {
    prosCount: 2500,
...
//# sourceMappingURL=services-mocks.js.map